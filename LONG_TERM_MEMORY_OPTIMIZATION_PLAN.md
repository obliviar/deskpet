...
- √ 处理现有 10 个未提交文件：逐项评审、补测后提交，或从正式计划中排除。  
  成果：已将它们标记为阶段 0 之前的抽取/时间/召回实验改动并纳入完整回归；本阶段没有覆盖或回滚这些文件。当前仍未提交到 Git，因此“版本提交”不在本次完成范围内。
...
**阶段状态：** × 未完成（已完成 3/8）；本轮完成高精度写入门的首个可运行切片，V3 仍为正式召回源。
...
- × 高风险字段和低置信冲突进入询问/隔离区。  
  已完成部分：自动 secret、低证据、低置信和无依据冲突不进入 V3 正式召回，只作为带原始 Episode、评分、原因和版本的 V4 quarantined 候选保存；删除源聊天会清除候选正文和原始 Episode。尚缺候选审核/询问界面，因此本项保持 `×`。
- × 引入幂等重处理和 extractor/verifier/policy 版本。  
  已完成部分：已记录 extractor/verifier/policy 版本，重复且无新证据的候选为 NOOP；尚缺可断点、可批量、可比较旧新策略的后台重处理器，因此本项保持 `×`。
...
| 阶段 | 优先级 | 依赖 | 主要成果 | 当前状态 |
|---|---|---|---|---|
| 0 基线与工程卫生 | P0 | 无 | 已备份并完成全回归；冻结盲测与 evals 入库待后续独立完成 | × 未完成（已完成 4/6） |
| 1 V4 权威模型与生命周期 | P0 | 阶段 0 | 证据、结构化版本、双时态、加密事件日志、依赖级联、彻底清除、差异审计、kill switch | √ 已完成（8/8）并通过阶段验收 |
| 2 高质量写入门 | P0 | 阶段 1 核心 schema | 已完成双通道候选、独立本地 verifier 和七类写入决策；上下文、全面归一化、审核、重处理与长消息队列待完成 | × 未完成（已完成 3/8） |
| 3 混合分层动态召回 | P0/P1 | 阶段 1；可与阶段 2 后半并行 | 语义、时间、多跳、证据包 | × 未完成 |
