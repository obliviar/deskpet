...
- × 将真实本地 embedding 变成经过首次安装说明的推荐默认项；保留无模型降级。
  ...
  - √ 已完成后台准备与恢复：旧记忆按批嵌入、批次持久化、让出事件循环、可中断并从已完成内容哈希继续；模型安装或重新启用时继续由哈希路径回答，只有 `pending=0` 才原子重建运行时并启用 BGE。
  - √ 已完成生命周期级联：正文编辑/REFINE 使旧模型向量失效；容量淘汰、forget、clear 和 purge 删除派生向量；不可逆 purge 额外压实并覆盖受管侧索引备份，清除后残留审计包含侧索引。
  - √ 已完成模型完整性基础：ready 空标记已替换为版本化 manifest；固定模型/revision/dtype/运行库/向量维度，逐文件记录大小和 SHA-256；正式启用前进行全文件哈希校验，并用 `local_files_only` 禁止联网完成冷 pipeline 加载、512 维有限值/归一化/重复一致性探针。缺失、同尺寸篡改、运行库不兼容、路径越界或加载失败均不得启用 BGE，只降级本地哈希。
...
#### 阶段 3 第二个切片评估：语义模型安全切换基础（2026-08-14）

| 评估维度 | 结果 | 判定 |
|---|---|---|
| 向量空间隔离 | BGE 准备期间不覆盖 V3 当前向量；召回仅使用与活动模型指纹一致的原向量或侧索引向量 | 通过 |
| 前台冻结防护 | 桌面 BGE 路径关闭前台批量懒升级；旧事实未准备时不会在一次召回中逐条调用 BGE | 通过 |
| 原子启用与回退 | 安装/启用先完成侧索引，`pending=0` 后才切换；失败保留 local-hash 运行时；非 BGE 活动模型仍保留原兼容行为 | 通过 |
| 中断恢复 | 5,000 条测试在 256 条后主动中断，第二次只处理剩余 4,744 条；总文档嵌入调用严格为 5,000 次 | 通过；使用确定性假 embedder，不代表真实 BGE 耗时 |
| 切换后在线调用 | 5,000 条准备完成后切换模型，首次召回只调用一次查询 embedding，没有文档前台重嵌入 | 通过 |
| 加密和明文边界 | 独立快照与增量日志均经 AES-256-GCM；测试确认快照/日志不包含记忆正文 | 通过 |
| 内容与删除一致性 | 内容哈希阻止编辑后的陈旧向量被读取；编辑、淘汰、普通删除、清空和彻底清除均有级联 | 通过 |
| 自动回归 | memory 新增专项/压力测试；core 6/6；memory、Electron Node/Vue 类型检查、Electron production build、六次隔离启动与零残留 purge 冒烟通过；损坏侧索引仅降级语义检索，不关闭长期记忆 | 通过 |
| 真实语义质量 | 尚未使用已冻结且未参与开发的中文改写集比较 BGE 与 local-hash，也未记录目标硬件真实模型冷/热延迟 | 未通过，保持 `×` |

**本切片结论：** 已消除“开启真实语义模型后首次召回同步重嵌入全部旧记忆”的主要工程风险，并建立可加密、可恢复、可回退、可级联删除的双模型准备路径。这是“真实本地 embedding 推荐默认化”的必要基础，但不是完整验收；阶段 3 仍为 `×`，完整计划项仍为 2/8。

#### 阶段 3 第三个切片评估：模型完整性与独立 BM25 基础（2026-08-15）

| 评估维度 | 结果 | 判定 |
|---|---|---|
| BM25 独立性 | 召回查询不再接收或重新分词全库正文；索引自行维护 posting、TF、DF、文档长度和当前/历史语料统计，独立返回候选排名供 RRF 使用 | 通过 |
| BM25 等价与隔离 | 专项测试逐项对照旧暴力 BM25 分数；owner、agent、session、当前/历史分片以及更新删除无陈旧 posting | 通过 |
| 20,000 条回归 | 最终全仓测试中 12 次完整自适应召回平均 64 ms、P95 82 ms、最大 82 ms；候选 80、评估 16、注入 10，继续满足 P95 <100 ms | 通过；该耗时仍包含线性向量通道 |
| 模型静态完整性 | manifest 身份/运行库/维度校验、逐文件大小和 SHA-256、路径边界及普通文件检查；同尺寸篡改可检出 | 通过 |
